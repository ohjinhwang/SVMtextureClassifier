...
for i = 1:Ncv
    trainIndex = run ~= i; % validation set = i, remaining 4 sets are training sets
    trainData = data(trainIndex,:);
    trainLabel = label(trainIndex,:);
    
    testIndex = run == i; % validation set = i
    testData = data(testIndex,:);
    testLabel = label(testIndex,:);
    % Train the model on the selected parameters
    model = svmtrain(trainLabel, trainData, cmd);
    %model = fitcsvm(trainData,trainLabel,'Cost',0.03125,'KernelFunction','rbf',);
