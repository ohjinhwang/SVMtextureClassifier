...
trainLabel_int(trainLabel == 0,1) = 1;
trainLabel_int(trainLabel == 1,1) = 2;
...
evalLabel = trainLabel_int(evalIndex,:);
...
model = ovrtrainBot(trainLabel_int, trainData, bestParam);
% classify the test data set based on the svm model
[predict_label, accuracy, decis_values] = ovrpredictBot(testLabel_int, testData, model);
...
testLabel_int(testLabel == 0,1) = 1;
testLabel_int(testLabel == 1,1) = 2;
colorPlot = colorList(testLabel_int,:);
figure; 
